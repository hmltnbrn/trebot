...
exports.getRandomQuestion = async () => {
  ...
  const collection = client
    ...
    .collection(process.env.MONGO_CLUE_COLLECTION);
...
exports.increaseContestantScore = async (guildId, member, value) => {
  ...
  const collection = client
    .db(process.env.MONGO_DATABASE)
    .collection(process.env.MONGO_CONTESTANT_COLLECTION);
  ...
  const contestant = await collection.findOneAndUpdate(
    ...
    {
      $inc: { score: value, correct_answers: 1 },
      $set: { user_tag: member.displayName },
    },
...
exports.decreaseContestantScore = async (guildId, member, value) => {
  ...
  const contestant = await collection.findOneAndUpdate(
    { server_id: guildId, member_id: member.id },
    {
      $inc: { score: -1 * value, incorrect_answers: 1 },
      $set: { user_tag: member.displayName },
    },
    { returnDocument: 'after', upsert: true },
  );
...
exports.getContestants = async (guildId) => {
  ...
  const contestants = await collection
    .find({ server_id: guildId })
    .sort({ score: -1 })
    .toArray();
  client.close();
